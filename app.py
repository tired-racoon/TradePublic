    ...
    while True:
        ...
        if week_day in [5, 6]:
            ...
            for elem in tiks.keys():
                ...
                if tiks[elem] in [-1, -2]:
                    continue
                predictions[df[df['ticker'] == elem]['figi'].values[0]] = preds[tiks[elem]][1]  #predictions: figi-prediction
            data_to_display['info'] = 'Predictions are done </br>'
            sell_resps = check_stocks_to_sell(df, actives, acc_id, token, predictions)
            data_to_display['info'] = 'Selling stocks </br>'
            time.sleep(300)
            ...
            data_to_display['info'] = 'Unfinished orders are cancelled </br>'
